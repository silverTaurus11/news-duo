...
**Tidak ada kode yang mengecek "ini Duo atau bukan".** Tidak ada `if isDuo`, tidak ada model perangkat, tidak ada dua layout. Ada satu hierarki view, dan perbedaannya diputuskan oleh **sistem** lewat horizontal size class, sesuai HIG (layar luar = *compact width*, layar dalam = *regular width*).
...
**Pose (tertutup, terbuka, terlipat sebagian) hanya bisa diatur dari Device Hub.** `simctl` tidak punya perintah untuk itu. Di Device Hub Xcode 27.2 beta saya **tidak menemukan kontrolnya** **(diverifikasi)**:

- Menu Device, Controls, dan View, toolbar, menu "…", serta inspector Settings dan Info tidak punya perintah lipat atau buka.
- Ikon toolbar yang berbentuk jendela dengan panel kecil adalah **Resize Mode**. Ia hanya mengubah ukuran scene; bar tetap horizontal, jadi bukan pose Duo.
- Rotate (⌘←/⌘→) memutar layar luar ke lanskap dan tidak pernah menyalakan layar dalam.
- Tonjolan di tepi kanan gambar perangkat adalah **tombol samping** (side button); menyeretnya mengunci perangkat.
- Device Hub dari Xcode 27.1 menampilkan ikon-ikon berbentuk perangkat tambahan di bar bawah yang mungkin pilihan pose. Ini belum diverifikasi.

Device Hub ada di `Xcode.app/Contents/Applications/DeviceHub.app`. Ikon dua panah di pojok kanan atas jendela kecilnya memperluas jendela ke tata letak penuh (daftar perangkat, toolbar, inspector).
...
| Sudah | Belum |
|---|---|
| Layar luar (tertutup) di simulator Duo: tab bar dan toolbar (bendera) tampil di bar vertikal kanan, daftar satu kolom, data live; konten menyentuh keempat tepi layar tanpa letterbox | Layar dalam (terbuka): dua pane list + detail. **Kontrol untuk membuka perangkat simulasi tidak ditemukan** di Device Hub (Xcode 27.2 beta); lihat [persiapan alat](#1-persiapan-alat) |
| Tombol Save/Share, Done, dan negara memakai ikon + judul | Pose terlipat sebagian dan perilaku di area lipatan |
...
Sampai layar dalam bisa dibuka, pengganti terdekat adalah iPad Pro 13" (lebar regular): ia sudah menampilkan dua pane list + detail dengan benar.
